...
mem_init_mp(void) {
    ...
    size_t i;
    for (i = 0; i < NCPU; ++i) {
        boot_map_region(kern_pgdir, KSTACKTOP - (i + 1) * KSTKSIZE - i * KSTKGAP, KSTKSIZE, ROUNDDOWN(PADDR(percpu_kstacks[i]), PGSIZE), PTE_W);
    }
}
...
void page_init(void) {
    ...
    // free pages!
    size_t   i;
    uint32_t use_end = PADDR(boot_alloc(0));
    pages[0].pp_ref = 1;
    for (i = 1; i < npages_basemem; ++i) {
        if (i * PGSIZE <= MPENTRY_PADDR && MPENTRY_PADDR < (i + 1) * PGSIZE)
            pages[i].pp_ref = 1;
        else {
            pages[i].pp_ref = 0;
    ...
    for (i = IOPHYSMEM / PGSIZE; i < use_end / PGSIZE; i++) {
        pages[i].pp_ref = 1;
    }
    for (i = use_end / PGSIZE; i < npages; ++i) {
        if (i * PGSIZE <= MPENTRY_PADDR && MPENTRY_PADDR < (i + 1) * PGSIZE)
            pages[i].pp_ref = 1;
        else {
            pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags) {
    ...
    struct PageInfo* pa;
    if (NULL == page_free_list) return NULL;
    pa = page_free_list;
...
pgdir_walk(pde_t* pgdir, const void* va, int create) {
    ...
    pgtab = &pgdir[PDX(va)];
    if (NULL == pgtab) return NULL;
    if (((*pgtab) & PTE_P) == 0) {
        if (create == false) return NULL;
        pa = page_alloc(ALLOC_ZERO);
        if (NULL == pa) return NULL;
        pa->pp_ref += 1;
        *pgtab = page2pa(pa) | PTE_U | PTE_W | PTE_P;
    }
    return (pte_t*)KADDR(PTE_ADDR((*pgtab))) + PTX(va);
}
...
void page_remove(pde_t* pgdir, void* va) {
    ...
    struct PageInfo* pa;
    pte_t*           pte_store;
    pa = page_lookup(pgdir, va, &pte_store);
    if (NULL == pa) return;
    page_decref(pa);
    *pte_store = 0;
